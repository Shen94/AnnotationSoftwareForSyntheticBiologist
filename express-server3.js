...
const testFolder = './RealWork/TestData_Date';
...
for (var t = 0;  t < Directory.length; t++) {
 ...
 for (var l = 0; l < file_sort.length; l++) {
        ...
        var json_content = JSON.parse(contents2);
            ...
            for (var i= Start_Line ; i<=End_Line ;i++){
             var File_Init=getLine(contents,i);
                ...
                fs.writeFileSync(__dirname + "/" + Directory_Converted[t] + "/" + File_Init[0] + '.txt', '{"'+ File_Init[0]+ '":"' + json_content[File_Init[0]]+'",' , function (err) {
                  
                    if (err) return console.log(err);
                            });
                
                fs.appendFileSync(__dirname + "/" + Directory_Converted[t] + "/" + File_Init[0] + '.txt', '"Location": "' + File_Init[0] + '",',function (err) {
                  
                    if (err) return console.log(err);
                            });
                
                fs.appendFileSync(__dirname + "/" + Directory_Converted[t] + "/" + File_Init[0] + '.txt', '"Time":[' + time_new + "],", function (err) {
      ...
      fs.appendFileSync(__dirname + "/" + Directory_Converted[t] + "/" + File_Init[0] + '.txt', '"Fluroescence":['+FReadings2 + "],", function (err) {
        ...
        fs.appendFileSync(__dirname + "/" + Directory_Converted[t] + "/" + File_Init[0] + '.txt', '"Absorbance":['+AReadings2 +"]}", function (err) {
